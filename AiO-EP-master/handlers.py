    ...
    # Xử lý các callback có pattern
        ...
        if result and result.get('success'):
            ...
            admin_notification = (
                ...
                f"  • Giá bán: {result['price']:,} {config.CURRENCY}\n"
                f"  • Số dư còn lại: {result['new_balance']:,} {config.CURRENCY}\n\n"
                f"⏱️ Thời gian: {current_time}\n"    
                f"📊 Mã giao dịch: TX{user_id}{int(time.time())}"
            )
            
            # Thử gửi với Markdown, nếu lỗi thì gửi dạng văn bản thường
            try:
                notify_admins(bot, admin_notification, parse_mode="Markdown")
            except Exception as e:
                logger.error(f"Error sending Markdown notification: {e}")
                # Gửi lại không có định dạng Markdown
                plain_notification = (
                    f"🔔 THÔNG BÁO GIAO DỊCH THÀNH CÔNG!\n\n"
                    f"👤 *Thông tin khách hàng:*\n"
                    f"  • Tên: @{username}\n"
                ...
                notify_admins(bot, plain_notification)
        else:
